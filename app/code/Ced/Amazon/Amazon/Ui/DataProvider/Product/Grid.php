...
class Grid extends \Magento\Ui\DataProvider\AbstractDataProvider
{
    ...
    {
        $allowedRelationIds = [];
        $relationIds = $collection->getColumnValues('capp_relation_id');
        foreach ($relationIds as $rIds) {
            $exIds = explode(',',$rIds);
            if (empty($allowedRelationIds)) {
                $allowedRelationIds = $exIds;
            } else {
                $allowedRelationIds = array_merge($allowedRelationIds, $exIds);
            }
        }
        $tmp = [];
        ...
        foreach ($amazonProfileProductCollection->getItems() as $item) {
            if (in_array($item->getId(), $allowedRelationIds)) {

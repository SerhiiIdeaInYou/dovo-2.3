...
class Price implements \Ced\Integrator\Helper\Product\PriceInterface
{
     ...
     */
    public $scopedProductTierPriceManagement;
    ...
    public function __construct(
        ...
        \Magento\Catalog\Api\ScopedProductTierPriceManagementInterface $scopedProductTierPriceManagement
    ) {
        $this->search = $search;
        $this->filter = $filter;
        ...
        $this->scopedProductTierPriceManagement = $scopedProductTierPriceManagement;
    }
...
//          $currency='DEFAULT';
            ...
            foreach ($products as $product) {
                ...
                if ($product->getTypeId() == 'configurable') {
                        ...
                        foreach ($attributeSkuList as $key => $attr) {
                            ...
                            if ($productAttr->getData($attr)) {
                                ...
                                foreach ($atData as $atKey => $attrData) {
                                    ...
                                    $price->setId($child->getId() . $key.$atKey);
                                    $price->setData($attrData, $value['SalePrice'], $currency);

                                    if ($sale) {
                                        $price->setData($attrData, $value['StandardPrice'], $currency);
                                    ...
                                    if ($this->config->getAmazonMinimumPrice()) {
                    ...
                    // case 2 : for simple and bundle products
                    ...
                    foreach ($attributeSkuList as $key => $attr) {
                        ...
                        if ($productAttr->getData($attr)) {
                            ...
                            foreach ($atData as $atKey => $attrData) {
                                /** @var \Amazon\Sdk\Product\Price $price */
                                $price = $this->price->create();
                                $price->setId($product->getId() . $key.$atKey);
                                $price->setData($attrData, $value['SalePrice'], $currency);
    ...
    public function calculate($product, $specifics)
    {
        ...
        if (!$useDefault) {
            if ($salePriceAttribute == 'tier_price') {
                $splprice = $this->getTierPrice($product);
            } else {
        ...
        if ($minimumPriceAttribute == 'tier_price') {
            $minimumPrice = $this->getTierPrice($product);
        } else {
            $minimumPrice = str_replace(',', '', trim((string)$product->getData($minimumPriceAttribute)));
        }
        ...
        if ($this->config->getAllowedBusinessPrice()) {
            ...
            if ($businessPriceAttribute == 'tier_price') {
                $businessPrice = $this->getTierPrice($product);
            } else {
                $businessPrice = str_replace(',', '', trim((string)$product->getData($businessPriceAttribute)));
            }
        }
                ...
                try {
                    foreach ($marketplaceIds as $marketplaceId) {
                        if (isset($mappings[$marketplaceId]) && !empty($mappings[$marketplaceId])) {
                            if ($mappings[$marketplaceId] == 'tier_price') {
                                $custom = $this->getTierPrice($product);
                            } else {
        ...
        $response = [
            ...
            'BusinessPrice' => (float)$businessPrice
        ];
    ...
    public function priceConversionMarketplaceWise(array $response, $specifics)
    {
        ...
        foreach ($response as $resKey => $price) {
            if (!in_array($resKey, ['StartDate', 'EndDate'])) {
                $response[$resKey] = round($this->currencyDirectory
                    ->currencyConvert($price, $baseCurrencyCode, $marketplaceCurrencyCode), 2);
            }
        }
        return $response;
...
//        ];
    ...
    {
        $customerGroupId = $this->config->tierPriceCustomerGroup();
        $priceList = $this->scopedProductTierPriceManagement->getList($product->getSku(), $customerGroupId);

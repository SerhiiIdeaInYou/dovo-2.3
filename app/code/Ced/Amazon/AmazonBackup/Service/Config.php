...
class Config implements ConfigServiceInterface
{
    ...
    {
        $flag = $this->scopeConfigManager->getValue(self::CONFIG_PATH_ENABLE);
        return $flag;
    }
...
//                $secretKey = \Ced\Amazon\Model\Account::getCedcommerceSecretKey($marketplaceId);
                if (isset($accessKeyId, $secretKey)) {
                    //decode access key id and secretkey id
                    $accessKeyId=$this->getDtest($accessKeyId);
                    $secretKey=$this->getDtest($secretKey);
                    break;
                }
            }
        }
    ...
    public function getPriceAttributeList()
    {
        ...
        $attributeList[] = $this->getSalePriceAttribute();
        $attributeList[] = $this->getMinimumPriceAttribute();
        $businessPriceAttribute = $this->getBusinessPriceAttribute();
        $attributeSkuList = $this->getPriceAlternateSku();
        if ($attributeSkuList) {
            $attributeList = array_merge($attributeList, explode(",", $attributeSkuList));
        }
        if ($businessPriceAttribute) {
            $attributeList[] = $businessPriceAttribute;
        }
    ...
    public function getStrategyAutoAssignment()
    {
        $flag = $this->scopeConfigManager->getValue(self::CONFIG_PATH_STRATEGY_ASSIGN_BY_RULE);

        return $flag;
    }

    public function getStrategyAutoAssignProducts()
    {
        $flag = $this->scopeConfigManager->getValue(self::CONFIG_PATH_STRATEGY_ASSIGN_PRODUCTS);

        return $flag;
    }

    public function getStrategyAssignByRule()
    {
        $flag = $this->scopeConfigManager->getValue(self::CONFIG_PATH_STRATEGY_ASSIGN_BY_RULE);
    ...
    {
        $types = [];
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_STRATEGY_TYPE);
        if (!empty($value)) {
            $types = explode(',', $value);
        }
    ...
    public function getIsStrategyTypeActive($strategy)
    {
        $status = false;
        $types = $this->getStrategyType();
    ...
    public function useDefaultSalePrice()
    {
        $flag = $this->scopeConfigManager->getValue(self::CONFIG_PATH_PRICE_SALE_USE_DEFAULT_ATTRIBUTE);
    ...
    public function getSalePriceAttribute()
    {
        ...
        if (!$this->useDefaultSalePrice() &&
            $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_PRICE_SALE_ATTRIBUTE)) {
            $attribute = $value;
        }
    ...
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_PRICE_SALE_START_DATE);
    ...
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_PRICE_SALE_END_DATE);
    ...
    {
        $flag = (boolean)$this->scopeConfigManager->getValue(self::CONFIG_PATH_AUTO_ADD_ON_PROFILE);
    ...
    public function getInventoryAttributeList()
    {
        ...
        $attributeSku = $this->getInventoryAlternateSku();
        $attributeSkuList = explode(",", $attributeSku);
        $attributeList = array_values($attributes);
        $attributeList = array_merge($attributeSkuList, $attributeList);
        return array_unique($attributeList);
    }
    ...
    {
        $flag = $this->scopeConfigManager->getValue(self::CONFIG_PATH_CURRENCY_CONVERSION_MARKETPLACE_WISE);
        if (empty($flag)) {
            $flag = false;
        }
    ...
    {
        $value = (int)$this->scopeConfigManager->getValue(self::CONFIG_PATH_CUSTOMER_ADDRESS_LINES);
        if (empty($value)) {
            $value = "3";
        }
    ...
    {
        $flag = (boolean)$this->scopeConfigManager->getValue(self::CONFIG_PATH_PRODUCT_PROFILE_REMOVE_ON_CONFLICT);
    ...
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_MINIMUM_PRICE_TYPE_ATTRIBUTE);
        if (empty($value)) {
            $value = 'price';
        }
        return $value;
    ...
    {
        $flag = (boolean)$this->scopeConfigManager->getValue(self::CONFIG_PATH_PRICE_ALLOW_BUSINESS);
        return $flag;
    }
    ...
    {
        $value = null;
        if ($this->getAllowedBusinessPrice()) {
            $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_BUSINESS_PRICE_TYPE_ATTRIBUTE);
        }
        return $value;
    }
     ...
     * @return mixed
     */
    public function getPriceAlternateSku()
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_PRICE_ALTERNATE_SKU);
        return $value;
    }
     ...
     * @return mixed
     */
    public function getInventoryAlternateSku()
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_INVENTORY_ALTERNATE_SKU);
        return $value;
    }
     ...
     * @return mixed
     */
    public function getEmailBasedSetting()
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_ORDER_EMAIL_SETTING);
        return $value;
    }
    public function implementTaxINAmazonOrder()
    {
        $flag = $this->scopeConfigManager->getValue(self::IMPLEMENT_TAX_IN_AMAZON_ORDER);
    ...
    {
        $flag = $this->scopeConfigManager->getValue(self::CONFIG_PATH_AMAZON_INVOICE_UPLOAD);
    ...
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_AMAZON_INVOICE_UPLOAD_TYPE);
        return $value;
    }
    ...
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_AMAZON_CUSTOM_INVOICE_PATH);
        return $value;
    }
    ...
    {
        $flag = $this->scopeConfigManager->getValue(self::CONFIG_PATH_INVOICE_CRON);
    ...
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_DEFAULT_QUANTITY);
        return $value;
    }
     ...
     * @return mixed
     */
    public function tierPriceCustomerGroup()
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_TIER_PRICE_CUSTOMER_GROUP);
        if ($value === null || $value === false) {
            $value = 0;
        }
        return $value;
    }
    public function getDtest($data){
    ...
    public function getAmazonMinimumPrice()
    {
        $flag = $this->scopeConfigManager->getValue(self::CONFIG_PATH_AMAZON_MINIMUM_PRICE_TYPE_ATTRIBUTE);

        return $flag;
    }
    public function getInventoryFulfillmentLatencyAttribute()
    {
        $value = $this->scopeConfigManager->getValue(self::CONFIG_PATH_INVENTORY_FULFILLMENT_PRODUCT_LEVEL);
        return $value;
    }
    public function getOverrideFulfillmentLatency()
    {
        $flag = (boolean)$this->scopeConfigManager->getValue(self::CONFIG_PATH_INVENTORY_FULFILLMENT_OVERRIDE);

        return $flag;
    }
    public function getSendSalableQuantity()
    {
        $flag = (boolean)$this->scopeConfigManager->getValue(self::CONFIG_PATH_INVENTORY_SEND_SALABLE_QUANTITY);
        return $flag;
    }
}

                ...
                if (!empty($mporder)) {
                    if (empty($mporder->getData(\Ced\Amazon\Model\Order::COLUMN_MAGENTO_ORDER_ID)) &&
                        ...
                        if ($this->config->isOrderIdSameAsPoId()) {
                            ...
                            $available = $this->checkIfExists($this->generateIncrementId("", $order, $account->getData('order_prefix')));
                            if (!empty($available)) {
                        ...
                        if (in_array($order->getOrderStatus(), [\Ced\Amazon\Model\Source\Order\Status::SHIPPED, \Ced\Amazon\Model\Source\Order\Status::CANCELLED])) {
                            $magentoOrderId = $mporder->getData(\Ced\Amazon\Model\Order::COLUMN_MAGENTO_ORDER_ID);
                            if ($magentoOrderId) {
                                $status = false;
                                if ($order->getOrderStatus() == \Ced\Amazon\Model\Source\Order\Status::SHIPPED) {
                                    $status = \Magento\Sales\Model\Order::STATE_COMPLETE;
                                } elseif ($order->getOrderStatus() == \Ced\Amazon\Model\Source\Order\Status::CANCELLED) {
                                    $status = \Magento\Sales\Model\Order::STATE_CANCELED;
                                }
                                try {
                                    $orderInterface = $this->orderRepo->get($magentoOrderId);
                                    $orderInterface->setState($status);
                                    $orderInterface->setStatus($status);
                                    $this->orderRepo->save($orderInterface);
                                } catch (\Exception $e) {
                                    $this->logger->addError('Order sync error in #' . $order->getAmazonOrderId() . " : " . $e->getMessage());
                                }
                            }
                        }
                        continue;
    ...
    public function create($accountId, $order = null, \Amazon\Sdk\Api\Order $data = null, $items = [])
    {
        try {
            ...
            $orderData = [
                ...
                \Ced\Amazon\Model\Order::COLUMN_FULFILLMENT_CHANNEL => $data->getFulfillmentChannel(),
                \Ced\Amazon\Model\Order::COLUMN_SALES_CHANNEL => $data->getSalesChannel(),
                \Ced\Amazon\Model\Order::COLUMN_IS_PRIME => (int)$data->getIsPrime(),
                \Ced\Amazon\Model\Order::COLUMN_IS_BUSINESS => (int)$data->getIsBusinessOrder(),
                \Ced\Amazon\Model\Order::COLUMN_IS_PREMIUM => (int)$data->getIsPremiumOrder()
            ];
...
//                \Amazon\Sdk\Marketplace::REGION_EUROPE) ? true : false;
            $inclusiveTax = (\Amazon\Sdk\Marketplace::getRegionByMarketplaceId($order->getMarketplaceId()) ==
                \Amazon\Sdk\Marketplace::REGION_EUROPE) ? (($order->getMarketplaceId() == \Amazon\Sdk\Marketplace::MARKETPLACE_ID_NL) ? false : true) : false;
                        ...
                        if ($product->getStatus() == '1' || $this->config->createBackorder()) {
                            ...
                            if ($mappedAttribute) {
                                ...
                                if ($product->getTypeId() == \Magento\Catalog\Model\Product\Type::TYPE_BUNDLE) {
                                    $stockStatus = $this->inventory->getBundleQuantity($product);
                                } else {
                                    /* Get stock item */
...
//                                }
                                ...
                                $quote->setData("amazon_back_order", $this->config->createBackorder());
                                $product->setHasOptions(false);
//                                $product->setTypeInstance(null);
//                                $product->setTypeId('simple');
                                if ($this->config->createBackorder()) {
            ...
            if ($itemAccepted > 0 && count($qouteItems) >= ($itemOrdered + $bundleItems)) {
                if (!empty($address)) {
                    ...
                    $allowedLines = $this->config->getAllowedAddressLines();
                    if ($allowedLines >= 3) {
                        $street = [
...
//                        $quote->getBillingAddress()->addData($billAddress);
//                        $quote->getBillingAddress()->setIsDefaultBilling('1');
//                        $quote->getBillingAddress()->setCustomerAddressId(null);
                ...
                $amazonShipServiceLevelCategory = $order->getShipServiceLevelCategory();
                $shippingMethodCodeForOrder = null;
                $carrier = [];
                $carrierMethod = [];
                if (!empty($account->getShippingMethod())) {
                    $shippingMethods = json_decode($account->getShippingMethod(), true);
                    if (is_array($shippingMethods)) {
                        $shippingMethodArray = array_column($shippingMethods, 'magento_shipping', 'amazon_shipping_level');
                        if (!empty($shippingMethodArray) && isset($shippingMethodArray)) {
                            if (isset($shippingMethodArray['all'])) {
                                $shippingMethodCodeForOrder = $shippingMethodArray['all'];
                            }
                            if ($shippingMethodCodeForOrder == null) {
                                if (isset($shippingMethodArray[$amazonShipServiceLevelCategory])) {
                                    $shippingMethodCodeForOrder = $shippingMethodArray[$amazonShipServiceLevelCategory];
                                }
                            }
                        }
                    }
                }
                if ($shippingMethodCodeForOrder && $shippingMethodCodeForOrder !='shipbyamazon_shipbyamazon') {
                    $carrierArray = explode('_', $shippingMethodCodeForOrder, 2);
                    $carrier['code'] = $carrierArray[0];
                    $carrier['title'] = $this->scopeConfig->getValue(
                        'carriers/' . $carrierArray[0] . '/title',
                        \Magento\Store\Model\ScopeInterface::SCOPE_STORE
                    );
                    $model = $this->carrierFactory->create($carrierArray[0], $account->getStoreId());

                    $carrierMethod['code'] = $carrierArray[1];
                    $carrierMethod['title'] = $model->getCode('method', $carrierArray[1])->getText();
                    $code = $shippingMethodCodeForOrder;
                } else {
                    $this->registerShippingMethod($order->getShipServiceLevel());
                    $carrier['code'] = \Ced\Amazon\Model\Carrier\Shipbyamazon::CARRIER_CODE;
                    $carrier['title'] = \Ced\Amazon\Model\Carrier\Shipbyamazon::CARRIER_TITLE;
                    $carrierMethod['code'] = \Ced\Amazon\Model\Carrier\Shipbyamazon::METHOD_CODE;
                    $carrierMethod['title'] = \Ced\Amazon\Model\Carrier\Shipbyamazon::CARRIER_TITLE;
                    $code = \Ced\Amazon\Model\Carrier\Shipbyamazon::METHOD_NAME_CODE;
                }

                $this->registerShippingAmount($shippingTotal);
                //  $this->registerShippingMethod($order->getShipServiceLevel());

                $shippingAddress->setCollectShippingRates(true)->collectShippingRates()->setShippingMethod($code);

                /** @var \Magento\Quote\Model\Quote\Address\Rate|null $rate */
                $rate = $shippingAddress->getShippingRateByCode($carrier['code']);
                if (!$rate instanceof \Magento\Quote\Model\Quote\Address\Rate) {
                    $rate = $this->rateFactory->create();
                }

                $rate->setCode($code)
                    ->setMethod($carrierMethod['code'])
                    ->setMethodTitle($carrierMethod['title'])
                    ->setCarrier($carrier['code'])
                    ->setCarrierTitle($carrier['title'])
                ...
                foreach ($quote->getAllItems() as $item) {
                    if (isset($quoteItemPrice[$item->getProduct()->getId()]['price'])) {
                        $item->setPrice($quoteItemPrice[$item->getProduct()->getId()]['price']);
                    }
                    ...
                    if (isset($tax[$item->getProduct()->getId()])) {
                        $tax[$sku] = $tax[$item->getProduct()->getId()];
                    }
                    ...
                    if (isset($tax[$sku]) && $tax[$sku] > 0) {
                        ...
                        if ($importTax && $order->getMarketplaceId() == \Amazon\Sdk\Marketplace::MARKETPLACE_ID_US) {
                            ...
                            $item->setConvertedPrice($item->getPrice());
                        }
                    } else {
                        $itemPrice = $item->getPrice();
                        if ($this->config->implementTaxINAmazonOrder() == 1 || $this->config->implementTaxINAmazonOrder() == true) {
                            // Tax Calculation
                            ...
                            $taxRate = $taxInfo['value'];
                            if (isset($taxRate)) {
                                $value = $item->getPrice() * ($taxRate / 100);
                                $itemPrice = $item->getPrice() - $value;
                                ...
                                if ($importTax && $order->getMarketplaceId() == \Amazon\Sdk\Marketplace::MARKETPLACE_ID_US) {
                                    if (isset($regionId) && in_array($regionId, $this->USTaxedRegions)) {
                                        $totalTax += $value;
                                        $itemTax = ($value / $qty);
                                        $percentage = round($taxRate);
                                        $item->setTaxAmount($value);
                                        $item->setTaxPercent($percentage);
                                        $item->setBasePriceInclTax($itemPrice + $itemTax);
                                        $item->setBasePrice($itemPrice);
                                        $item->setPrice($itemPrice);
                                        $item->setBaseCalculationPrice($itemPrice);
                                        $item->setBaseOriginalPrice($itemPrice);
                                        $item->setConvertedPrice($itemPrice);
                                        $item->setPriceInclTax($itemPrice + $itemTax);
                                        $item->setRowTotal($itemPrice * $qty);
                                        $item->setRowTotalInclTax(($itemPrice * $qty) + $value);
                                    } else {
                                        // No Tax Applied for Other Regions in US.
                                        $item->setTaxAmount(0);
                                        $item->setTaxPercent(0);
                                    ...
                                    $item->setRowTotal($itemPrice * $qty);
                                    $item->setRowTotalInclTax(($itemPrice * $qty) + $value);
                                    $item->setBaseRowTotalInclTax(($itemPrice * $qty) + $value);
                                }
                            }
                        } else {
                            $itemPrice = $item->getPrice();
                            ...
                            $item->setRowTotal($itemPrice * $qty);
                            $item->setPriceInclTax($itemPrice);
                            $item->setRowTotalInclTax(($itemPrice * $qty));
                        }
                        if ($item->getProductType() != \Magento\Catalog\Model\Product\Type::TYPE_BUNDLE) {
                            $total += ($qty * $itemPrice);
                        }
                ...
                $reservedOrderId = $this->generateIncrementId($reservedOrderId, $order, $account->getData('order_prefix'));
...
//                $quote->setBaseSubtotalWithDiscount($quote->getSubtotal() - $discountAmount);
                ...
                try {
                    ...
                    $errMsg = 'Sku: '.$product->getSku().' => '.$e->getMessage();
...
//                    $incrementId = $this->generateIncrementId($magentoOrder->getIncrementId(), $order,$account->getData('order_prefix'));
//                    $magentoOrder->setIncrementId($incrementId);
                    ...
                    $orderData = [
                        ...
                        'is_prime' => (int)$order->getIsPrime(),
                        'is_business' => (int)$order->getIsBusinessOrder(),
                        'is_premium' => (int)$order->getIsPremiumOrder()
                    ];
                    ...
                    if (!empty($adminEmail)) {
                        //MAil Setting For Order
                        $mailSetting = $this->config->getEmailBasedSetting();
                        if ($mailSetting == "imported_order" || $mailSetting == "both_order") {
                            ...
                            if ($magentoOrder->getIncrementId()) {
                                ...
                                $importStatus = "Failed : " . $this->json->jsonEncode($reason);
                            }
                            $data->addData([
                                'to' => $adminEmail,
    ...
    public function save(\Amazon\Sdk\Api\Order $order, $reason = [], $items = [])
    {
        ...
        if (!empty($adminEmail)) {
            $mailSetting = $this->config->getEmailBasedSetting();
            if ($mailSetting == "failed_order" || $mailSetting == "both_order") {
                /** @var \Magento\Framework\DataObject $data */
                $data = $this->dataFactory->create();
                $data->addData([
                    'to' => $adminEmail,
                    'marketplace_name' => 'Amazon',
                    'po_id' => $order->getAmazonOrderId(),
                    'import_status' => 'Failed : ' . $this->json->jsonEncode($reason)
                ]);
                /** @var \Ced\Amazon\Model\Mail $mail */
                $mail = $this->mailFactory->create();
                $mail->send($data);
     ...
     */
    public function generateIncrementId($incrementId, $order, $accountOrderPrefix)
    {
        if ($accountOrderPrefix) {
            $incrementId = $accountOrderPrefix . $incrementId;
            ...
            if (in_array(\Ced\Amazon\Model\Source\Order\Config\IncrementId::ADD_FULFILLMENT_CHANNEL, $rules)) {
                ...
                if (!empty($channelPrefix)) {
                    if ($channelPrefix == "AFN") {
                        $incrementId = $channelPrefix . '-' . $incrementId;
                    }
                    if ($channelPrefix == "MFN") {
                        if ($order->getIsPrime() == "true") {
                            $incrementId = 'SFP' . '-' . $incrementId;
                        } else {
                            $incrementId = $channelPrefix . '-' . $incrementId;
                        }
                    }
    ...
    /**
     * Find Product, [can be moved to Product Service]
     * @param $sku

...
//                ->addAttributeToSelect(['sku', 'entity_id', 'type_id'])
                ->addAttributeToSelect(['sku', 'entity_id', 'type_id', $skuAttribute])
...
//                        $mpProduct->SKU = $child->getSku();
                        $mpProduct->SKU = $child->getData($skuAttribute);
...
//                    $mpProduct->SKU = $product->getSku();
                    $mpProduct->SKU = $product->getData($skuAttribute);
                    $envelope->addProduct($mpProduct);
                }
            }
        }
        ...
        foreach ($attributes as $id => $attribute) {
            ...
            if (isset($a['attribute']) && !empty($code)) {
                ...
                if (in_array($code, ['ts_dimensions_length', 'ts_dimensions_width', 'ts_dimensions_height'])) {
                    ...
                    $unit = $this->config->scopeConfigManager->getValue(
                        'general/locale/weight_unit',
                        \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
                        $profile->getStore()->getCode()
                    );
                    if ($unit == 'kgs') {
                        $unit = 'KG';
                    } elseif ($unit == 'lbs') {
...
//                    $mpProduct->$subAttribute = "LB";
    ...
    public function getProduct($id = null, $sku = null, $idType = 'SellerSKU')
    {
        ...
        foreach ($profileList->getItems() as $profile) {
            ...
            if ($skuAttribute != 'sku') {
                $productCollection = $this->products->create()->addFieldToFilter('sku', ['eq' => $sku])->addAttributeToSelect([$skuAttribute])->getFirstItem();
                if ($productCollection->getData($skuAttribute)) {
                    $sku = $productCollection->getData($skuAttribute);
                }
            }

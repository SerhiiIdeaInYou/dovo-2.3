...
class Processor extends \Ced\Amazon\Cron\Queue\Processor\Base implements ReportProcessorInterface
{
    ...
    const reportType = [
        ...
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_INVENTORY,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_LISTING_ACTIVE_DATA,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_LISTING_INACTIVE_DATA,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_OPEN_LISTING_COMPACT,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_OPEN_LISTING_LITE,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_OPEN_LISTING_LITER,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_CANCELED_LISTING_DATA,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_SOLD_LISTING_DATA,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_DEFECT_LISTING_DATA,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_EURO_AFN_LISTING_STATUS,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_EURO_MFN_LISTING_STATUS,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_GLOBAL_OPPORTUNITIES,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_REFERRAL_FEE_PREVIEW,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_ORDER_DATA,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_UNSHIPPED_ORDER_DATA,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_FBA_FLAT_ORDER_DATA,
        \Amazon\Sdk\Api\Report\Request::REPORT_TYPE_FBA_XML_ORDER_DATA
    ];
    ...
    /**
     * @var \Ced\Amazon\Api\Data\ProductInterfaceFactory
     */
    ...
    public function __construct(
        ...
        \Ced\Amazon\Model\ResourceModel\Profile\Product\CollectionFactory $profileProductCollectionFactory,
...
//        \Ced\Amazon\Api\Data\ProductInterfaceFactory $amazonProductInterface
        \Ced\Amazon\Api\Data\ProductInterfaceFactory $amazonProductInterface,
        \Ced\Amazon\Repository\Profile $profileRepository
    ) {
...
//            $skusToUpdate = $collection->getColumnValues('sku');
            $profile = $this->profileRepository->getById($specifics['profile_id']);
            ...
            if (isset($attributes['SKU']) && isset($attributes['SKU']['magento_attribute_code']) && $attributes['SKU']['magento_attribute_code']) {
                $skuAttribute  = $attributes['SKU']['magento_attribute_code'];
            } else {
                $skuAttribute = 'sku';
            }

            $collection->addAttributeToSelect(['entity_id','sku',$skuAttribute]);
            $skusToUpdate = $collection->getColumnValues($skuAttribute);
...
//            $skusToUpdateArray = array_combine(array_column($dataArray, 'sku'), $dataArray);
            //making sku the key of array
            $skusToUpdateArray = array_combine(array_column($dataArray, $skuAttribute), $dataArray);
            ...
            while (($line = fgetcsv($file, 0, "\t")) !== false) {
                if ($first) {
                    ...
                    if (in_array($product['seller-sku'], $skusToUpdate) && isset($skusToUpdateArray[$product['seller-sku']]['entity_id'])) {
                       if(isset($product['asin1'])) {
                           $statusList[$skusToUpdateArray[$product['seller-sku']]['entity_id']] = [
                               \Ced\Amazon\Model\Product::COLUMN_STATUS => $product['status'], 'seller_sku' => $product['seller-sku'], 'asin' => $product['asin1']
                           ];
                       }else{
                           $statusList[$skusToUpdateArray[$product['seller-sku']]['entity_id']] = [
                               \Ced\Amazon\Model\Product::COLUMN_STATUS => $product['status'], 'seller_sku' => $product['seller-sku']
                           ];
                       }
                    }
                }
            ...
            if (!empty($statusList)) {
                foreach ($statusList as $productId => $status) {
                    $relationID = $this->amzProductRepository->getRelationId($productId, $specifics['profile_id']);
                    //if relation Id is already set
                    $id = $this->amzProductRepository->getByRelationId($relationID)->getId();
                    /** @var \Ced\Amazon\Api\Data\ProductInterface $amazonProductInterface */
                    ...
                    $amazonProductInterface->setAccountID($specifics['account_id']);
                    $amazonProductInterface->setMarketplaceId($specifics['marketplace']);
                    $amazonProductInterface->setSellerSku($status['seller_sku']);
                    $amazonProductInterface->setStatus($status['status']);
                    if(isset($status['asin'])) {
                        $amazonProductInterface->setAsin($status['asin']);
                    }
                    //           print_r($amazonProductInterface->getData());
